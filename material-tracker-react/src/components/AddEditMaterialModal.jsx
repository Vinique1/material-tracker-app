...
const AddEditMaterialModal = ({ material, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center p-4 z-50" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="relative mx-auto p-8 border w-full max-w-2xl shadow-lg rounded-xl bg-white dark:bg-gray-800"> {/* MODIFIED: Added dark background */}
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 mb-2">{material ? 'Edit Material' : 'Add New Material'}</h2> {/* MODIFIED: Added dark text */}
        <p className="text-gray-500 dark:text-gray-400 mb-8">Fill out the details below to add an item to the inventory.</p> {/* MODIFIED: Added dark text */}
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"><X /></button> {/* MODIFIED: Added dark hover text */}
        <form onSubmit={handleSubmit} noValidate>
          <div className="space-y-6">
            <div className="space-y-4 p-5 bg-slate-50 dark:bg-gray-700 rounded-lg border border-slate-200 dark:border-gray-600"> {/* MODIFIED: Added dark background and border */}
              <h3 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Item Details</h3> {/* MODIFIED: Added dark text */}
              <div>
                <label htmlFor="description" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label> {/* MODIFIED: Added dark text */}
          ...
          <div className="mt-8 pt-5 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3"> {/* MODIFIED: Added dark border */}
            <button type="button" onClick={onClose} className="px-6 py-3 text-sm font-semibold bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors">Cancel</button> {/* MODIFIED: Added dark styles */}
        ...
        {({ open }) => (
          <div className="relative">
            ...
            <Transition as={Fragment} show={open} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0" afterLeave={() => setQuery('')}>
              <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-gray-700 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm z-20"> {/* MODIFIED: Added dark background */}
                {items.length === 0 && query !== '' ? (<div className="relative cursor-default select-none py-2 px-4 text-gray-700 dark:text-gray-300">Nothing found.</div>) : /* MODIFIED: Added dark text */
                (items.map((item) => (
                    <Combobox.Option key={item} className={({ active }) => `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-blue-600 text-white' : 'text-gray-900 dark:text-gray-300'}`} value={item}> {/* MODIFIED: Added dark text */}
                ...
                {allowAddNew && (<Combobox.Option className="relative cursor-default select-none py-2 px-4 text-blue-600 font-semibold hover:bg-blue-50 dark:hover:bg-blue-900/20" value="add_new">+ Add New {label}</Combobox.Option>)} {/* MODIFIED: Added dark hover background */}
              </Combobox.Options>
            </Transition>
          </div>
        )}
