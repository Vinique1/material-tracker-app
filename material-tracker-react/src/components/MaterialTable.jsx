...
const MaterialTable = () => {
    ...
    const handleOpenTransactionModal = (type, material) => {
        setTransactionDetails({ type, material });
        setIsTransactionModalOpen(true);
    };

    const filteredMaterials = materials.filter(m =>
        m.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (m.poNumber && m.poNumber.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <>
                ...
                <div className="p-6 border-b border-gray-200 flex justify-between items-center flex-wrap gap-4">
                    ...
                    <div className="flex items-center space-x-4">
                        <div className="relative">
                            <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search materials..." className="pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <Search className="h-5 w-5 text-gray-400" />
                            </div>
                        </div>
                        {!currentUser.isViewer && (
                             <button id="add-material-btn" onClick={handleOpenAddModal} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center space-x-2">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredMaterials.map(material => {
                                ...
                                if (delivered >= material.expectedQty) {
                                    status = 'Fully Delivered'; statusColor = 'bg-green-100 text-green-800';
                                } else if (delivered > 0) {
                                    status = 'Partially Delivered'; statusColor = 'bg-yellow-100 text-yellow-800';
                                } else {
                                    status = 'Pending Delivery'; statusColor = 'bg-red-100 text-red-800';
                                }
                                return (
                                <tr key={material.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{material.poNumber || 'N/A'}</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap"><span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColor}`}>{status}</span></td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        {!currentUser.isViewer && <div className="flex items-center space-x-2">
                                            <button onClick={() => handleOpenTransactionModal('deliver', material)} className="text-green-600 hover:text-green-900">Deliver</button>
                                            <button onClick={() => handleOpenTransactionModal('issue', material)} className="text-yellow-600 hover:text-yellow-900">Issue</button>
                                            <button onClick={() => handleOpenEditModal(material)} className="text-blue-600 hover:text-blue-900"><Edit className="h-4 w-4" /></button>
                                            <button onClick={async () => {
                                                if (window.confirm('Are you sure?')) {
                                                    await deleteDoc(doc(db, `materials/${ADMIN_UID}/items`, material.id));
                                                }
            ...
            {isMaterialModalOpen && <AddEditMaterialModal material={editingMaterial} onClose={() => setIsMaterialModalOpen(false)} />}
